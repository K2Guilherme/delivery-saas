...
export default function App() {
  ...
  // 1. TELA INSTITUCIONAL (SaaS Landing Page)
  if (!storeSlug) {
    return (
        <div className="flex flex-col items-center justify-center p-6 text-center min-h-screen relative overflow-hidden" style={darkBackground}>
            <div className="relative z-10 max-w-2xl">
                <div className="inline-block p-4 rounded-3xl bg-white/5 border border-white/10 mb-8 backdrop-blur-xl shadow-2xl">
                    <Store size={64} className="text-purple-400" />
                </div>
                
                <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-200 to-gray-400">
                    SaaS Delivery
                </h1>
                
                <p className="text-xl text-gray-400 mb-10 font-medium leading-relaxed max-w-lg mx-auto">
                    A plataforma completa para gerir o seu delivery. Cardápio digital, pagamentos Pix e gestão fácil.
                </p>
                
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <button className="px-8 py-4 bg-white text-gray-900 rounded-full font-bold text-lg hover:scale-105 transition-transform flex items-center gap-2 shadow-lg shadow-white/10">
                        Criar minha Loja <ArrowRight size={20}/>
                    </button>
                    <button className="px-8 py-4 bg-white/10 text-white rounded-full font-bold text-lg hover:bg-white/20 transition-colors border border-white/10">
                        Entrar no Painel
                    </button>
                </div>

                <p className="mt-12 text-sm text-gray-600">
                    Já tem uma loja? Use o seu link direto para acessar.
                    <br/>Exemplo: <i>seuapp.com/#/sua-loja</i>
                </p>
            </div>
        </div>
    );
  ...
  // Loading
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={darkBackground}><Loader className="animate-spin text-white/50" size={40} /></div>;

  // Erro 404 Loja
  if (!storeData) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center" style={darkBackground}>
        <div className="bg-red-500/10 p-6 rounded-full mb-6 border border-red-500/20"><AlertCircle size={48} className="text-red-500"/></div>
        <h1 className="text-3xl font-bold text-white mb-2">Loja não encontrada</h1>
        <p className="text-gray-400 mb-8">O link que você acessou não existe.</p>
        <a href="/" className="px-6 py-3 bg-white/10 rounded-xl text-white hover:bg-white/20 transition">Voltar ao Início</a>
    </div>
  );
      ...
      <div className="font-sans transition-colors duration-500 pb-24" style={darkBackground}>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-start gap-4">
              <div className="flex-1">
                ...
                {storeDesc && <p className="text-white/80 text-sm mt-2 font-medium leading-relaxed max-w-md">{storeDesc}</p>}
                <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    {storeAddress && <div className="flex items-center gap-1.5 bg-black/30 px-3 py-1.5 rounded-lg border border-white/10 text-white/90 text-xs font-medium"><MapPin size={12} className="text-gray-300"/> {storeAddress}</div>}
                    {storeInsta && <div className="flex items-center gap-1.5 bg-black/30 px-3 py-1.5 rounded-lg border border-white/10 text-white/90 text-xs font-medium"><Instagram size={12} className="text-pink-400"/> {storeInsta}</div>}
                </div>
              </div>
              <div className="relative cursor-pointer group" onClick={() => {setIsCartOpen(true); setCheckoutStep('cart')}}>
                 <div className="bg-white/10 p-3 rounded-full hover:bg-white/20 transition-all border border-white/10 backdrop-blur-md"><ShoppingCart size={24} className="text-white"/></div>
        ...
        <div className="p-4 max-w-4xl mx-auto">
            ...
            <div className="relative mb-8 mt-4 group">
                ...
                <input type="text" placeholder="Pesquisar..." className={darkInput + " pl-12 font-bold"} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            
            {/* Categorias */}
            <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar mb-4">
                {categories.map(cat => (
                    <button key={cat.id} onClick={() => setActiveCategory(cat.nome)} className={`px-5 py-2.5 rounded-xl whitespace-nowrap font-bold transition-all border ${activeCategory === cat.nome ? 'text-white border-transparent shadow-[0_0_15px_rgba(0,0,0,0.5)] transform scale-105' : 'bg-gray-800/50 text-gray-400 border-gray-700 hover:bg-gray-700'}`} style={activeCategory === cat.nome ? {backgroundColor: primaryColor} : {}}>{cat.nome}</button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {filteredItems.map(item => (
                    <div key={item.id} className={`${glassCard} p-4 rounded-3xl flex gap-4 hover:bg-gray-800/60 transition-all duration-300 group`}>
                        <div className="w-28 h-28 flex-shrink-0 rounded-2xl overflow-hidden bg-gray-900 shadow-lg border border-white/5"><img src={item.image} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500"/></div>
                        <div className="flex-1 flex flex-col justify-between py-1">
                            <div><h3 className="font-bold text-white text-lg leading-tight mb-1">{item.name}</h3><p className="text-xs text-gray-400 font-medium line-clamp-2">{item.description}</p></div>
                            <div className="flex justify-between items-center mt-3"><span className="font-black text-xl text-gray-100">R$ {item.price.toFixed(2)}</span><button onClick={() => addToCart(item)} className="p-2.5 rounded-xl shadow-lg active:scale-90 transition-all text-white hover:brightness-110" style={{backgroundColor: primaryColor}}><Plus size={20} strokeWidth={3}/></button></div>
                        </div>
                    </div>
                ))}
            </div>
        ...
        {cartCount > 0 && (
            <div className="fixed bottom-6 left-4 right-4 z-30 max-w-4xl mx-auto"><button onClick={() => setCheckoutStep('cart')} className="w-full text-white p-1 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] backdrop-blur-xl transition-all hover:scale-[1.02] border border-white/20" style={{background: `linear-gradient(90deg, ${primaryColor}, ${secondaryColor})`}}><div className="bg-black/30 p-4 rounded-xl flex justify-between items-center border border-white/10"><div className="flex items-center gap-3"><div className="bg-white/10 p-2 rounded-lg"><ShoppingCart size={20}/></div><span className="text-sm font-bold uppercase tracking-widest text-gray-300">Ver Sacola</span></div><span className="text-2xl font-black tracking-tight">R$ {cartTotal.toFixed(2)}</span></div></button></div>
        )}
      </div>
    );
  }
          ...
          <div className="min-h-screen p-6 font-sans flex flex-col items-center" style={darkBackground}>
              <div className="w-full max-w-lg">
                  ...
                  <div className={`${glassCard} p-8 rounded-[30px]`}>
                      <h2 className="text-2xl font-black text-white mb-8 border-b border-white/10 pb-4">{checkoutStep === 'cart' ? 'Sua Sacola' : checkoutStep === 'address' ? 'Entrega' : 'Pagamento'}</h2>
                      
                      {checkoutStep === 'cart' && (
                          <div className="space-y-4">
                              {cart.length === 0 ? <p className="text-center text-gray-500 py-10 font-bold">Vazio</p> : cart.map(item => (<div key={item.id} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0"><div><p className="font-bold text-white">{item.name}</p><p className="text-gray-400 text-sm">R$ {item.price.toFixed(2)}</p></div><div className="flex items-center gap-3 bg-gray-900/50 p-1 rounded-lg border border-white/5"><button onClick={() => updateQuantity(item.id, -1)} className="p-2 rounded-md text-gray-400 hover:text-white"><Minus size={14}/></button><span className="font-bold w-4 text-center text-white">{item.quantity}</span><button onClick={() => updateQuantity(item.id, 1)} className="p-2 rounded-md text-white shadow-sm" style={{backgroundColor: primaryColor}}><Plus size={14}/></button></div></div>))}
                      ...
                      {checkoutStep === 'address' && (
                          <div className="space-y-3">
                              <input name="name" placeholder="Seu Nome" value={customer.name} onChange={handleInputChange} className={darkInput} /><input name="phone" placeholder="WhatsApp" value={customer.phone} onChange={handleInputChange} className={darkInput} /><div className="h-px bg-white/10 my-2"></div><input name="street" placeholder="Rua" value={customer.street} onChange={handleInputChange} className={darkInput} /><div className="flex gap-3"><input name="number" placeholder="Nº" value={customer.number} onChange={handleInputChange} className={`w-1/3 ${darkInput}`} /><input name="neighborhood" placeholder="Bairro" value={customer.neighborhood} onChange={handleInputChange} className={`w-2/3 ${darkInput}`} /></div>
                      ...
                      {checkoutStep === 'payment' && (
                          <div className="space-y-4">
                              <div className="bg-gray-800/50 p-6 rounded-2xl text-center mb-6 border border-white/5"><p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Total a Pagar</p><p className="text-4xl font-black text-white">R$ {cartTotal.toFixed(2)}</p></div>
                      ...
                      {checkoutStep === 'pix_display' && pixData && (
                          <div className="text-center">
                              <div className="bg-white p-4 rounded-2xl inline-block mb-6 relative"><img src={`data:image/jpeg;base64,${pixData.qr_code_base64}`} className="w-64 h-64 object-contain mx-auto"/></div>
                              <div className="flex items-center justify-center gap-2 mb-6 bg-gray-800 px-4 py-2 rounded-full w-max mx-auto border border-gray-700"><RefreshCw size={14} className="animate-spin text-blue-400"/><span className="text-xs font-bold text-gray-300 uppercase">Aguardando...</span></div>
